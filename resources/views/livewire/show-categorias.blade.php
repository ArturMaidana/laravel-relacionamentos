...
<div class="min-h-screen bg-gray-50">
    <div class="container mx-auto py-8 px-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <!-- Search/Create Form -->
            <form wire:submit.prevent="{{ $categoriaId ? 'update' : 'create' }}" class="flex space-x-4">
                <div class="flex-1">
                    <input
                        type="text"
                        name="name"
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($categorias as $categoria)
                        <tr class="hover:bg-gray-50 {{ $categoriaId === $categoria->id ? 'bg-yellow-50' : '' }}">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {{ $categoria->id }}
                            </td>

                            <td class="px-6 py-4 text-sm text-gray-900 max-w-md">
                                <p class="line-clamp-3">{{ $categoria->name }}</p>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 max-w-md">
                                <p class="line-clamp-3">{{ $categoria->status ? 'Ativo' : 'Inativo' }}</p>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                                ...
                                <button
                                    wire:click.prevent="delete({{ $categoria->id }})"
